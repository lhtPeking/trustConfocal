...
for (i = 0; i < imageTitles.length; i++) {
	...
	run("Duplicate...", "title=Backup0_" + i);
}

allTitles = getList("image.titles");

for (i = 0; i < allTitles.length; i++) {
	if(startsWith(allTitles[i], "Backup0_")){
		...
		run("Analyze Particles...", "size=0.005-Infinity circularity=0.4-1.0 display summarize add exclude");
...
saveAs("Results", "/Users/haotianli/Library/CloudStorage/OneDrive-Personal/MouseData/condensateID/SIM data/Table/SIM_Prox1.csv");
